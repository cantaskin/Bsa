...
public class CreateAiVoiceProjectCommandHandlerTests
{
    ...
    public async Task Handle_ShouldThrowException_WhenArtistNotFound()
    {
        ...
        var command = new CreateAiVoiceProjectCommand
        {
            ...
            Text = "Sample Text",
            ArtistId = Guid.NewGuid(),
            LanguageId = new Guid("8151ab74-7b00-4d3e-8fde-af3a394404b3"),
            UsageRightId = new Guid("20e68a3b-d5bc-4edd-39fb-08dcfe476432"),
            ProjectSelection = ProjectSelectionEnum.InstantCloneVoice
        };

        // IArtistService'in GetAsync metodunu doğru şekilde mock'layalım
        _artistServiceMock
            .Setup(a => a.GetAsync(
        ...
        //ArtistShouldExistWhenSelected metodunu da mock'layalım
        //_artistServiceMock
        //    .Setup(x => x.ArtistShouldExistWhenSelected(null))
        //    .ThrowsAsync(new BusinessException(ArtistsBusinessMessages.ArtistNotExists));
        ...
        Assert.Equal(ArtistsBusinessMessages.ArtistNotExists, exception.Message);
    }
